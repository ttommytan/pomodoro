...
function Countdown({ givenTime }) {
  ...
  const [urls, setUrls] = useState(['https://images.alphacoders.com/135/1354376.jpeg',tog,ngnl, kawa, ahri, 
                                    mia,])
...
/*
  useEffect(() => {
    const e = document.getElementById("everything");
    if (fullScreen) {
      if (e?.requestFullscreen) {
        e.requestFullscreen();
      } else if (e?.webkitRequestFullscreen) { // For Safari
        e.webkitRequestFullscreen();
      } else if (e?.msRequestFullscreen) { // For IE11
  ...
  return (
    <>
      <div
        ...
        <button onClick={() => {allFalse();setTopLeft(true);showButtons();hideButtons();}} className={`top-left${(!white) ? 'black' : ''}`}> </button>
        <button onClick={() => {allFalse();setTopMid(true);showButtons();hideButtons();}}className={`top-mid${(!white) ? 'black' : ''}`}> </button>
        <button onClick={() => {allFalse();setTopRight(true);showButtons();hideButtons();}} className={`top-right${(!white) ? 'black' : ''}`}> </button>
        <button onClick={() => {allFalse();setMidLeft(true);showButtons();hideButtons();}}className={`mid-left${(!white) ? 'black' : ''}`}></button>
        <button onClick={() => {allFalse();setMidMid(true);showButtons();hideButtons();}} className={`mid-mid${(!white) ? 'black' : ''}`}></button>
        <button onClick={() => {allFalse();setMidRight(true);showButtons();hideButtons();}}className={`mid-right${(!white) ? 'black' : ''}`}></button>
        <button onClick={() => {allFalse();setBotLeft(true);showButtons();hideButtons();}} className={`bot-left${(!white) ? 'black' : ''}`}></button>
        <button onClick={() => {allFalse();setBotMid(true);showButtons();hideButtons();}}className={`bot-mid${(!white) ? 'black' : ''}`}></button>
        <button onClick={() => {allFalse();setBotRight(true);showButtons();hideButtons();}} className={`bot-right${(!white) ? 'black' : ''}`}></button>

    <div id="everything" onMouseEnter={showButtons} onMouseLeave={hideButtons} className={gridContainerName()}>   
          <div onMouseEnter={showButtons} onMouseLeave={hideButtons} className={buttonName()}>
              <button onMouseEnter={()=> setPomoHovered(true)} onMouseLeave={()=> setPomoHovered(false)}
              className={`buttons${(pomo || doublePomo) && !breakHover ? 'active' : ''}${!white ? 'black' : ''}`} onDoubleClick = {handlePomoDoubleClick} 
              onClick = {handlePomoClick}
              >pomodoro</button>
              <button onDoubleClick={handleBreakDoubleClick} 
              className={`buttons${(!pomo && !doublePomo) && !pomoHover ? 'active' : ''}${!white ? 'black' : ''}`} onClick = {handleBreakClick}
              > {longBreak ? "long break" : "short break"}</button>
          </div>
          ...
          <div onMouseEnter={showButtons} onMouseLeave={hideButtons} className={buttonName()}>
              <button onClick={handlePause} className={`pause-button${!white ? 'black' : ''}`}>{pause ? "start" : "pause"}</button>
              <button onDoubleClick={()=> index >= urls.length - 1 ? setIndex(0) : setIndex(index+1)} className={`buttons${!white ? 'black' : ''}`}onClick = {() => {pomo ? setTime(1500) : setTime(300); setPause(true)}}>refresh</button>
          </div>    
      </div>
