...
function Countdown({ givenTime }) {
      ...
      intervalIdRef.current = setInterval(() => {
        ...
        if (temp >= time) {
          ...
          if (unlock) {
            ...
            setTimeout(() => {
              setTransition(false);
              setNextIndex((prevIndex) => (prevIndex + 1) % urls.length);
            }, 2000);
          }
  ...
  useEffect(() => {
    let e = document.getElementById("next-container");
    if (fullScreen) e?.requestFullscreen();
    else if (document.fullscreenElement) {
      document.exitFullscreen();
    }
  }, [fullScreen]);
  const hideButtons = () => {
    if (!pause) setVisablePause(false);
  };
  ...
  function formatTime() {
    ...
    if (favicon) {
      // Example condition to change the favicon
      if (time >= (2 * refreshTime) / 3) {
        favicon.href = "/start.svg";
      } else if (time < refreshTime / 3) {
        favicon.href = "/end.svg"; // Set to your specific icon when time is 12
      } else if (time < (2 * refreshTime) / 3) {
        favicon.href = "/half.svg"; // Default favicon
      }
    }
  ...
  useEffect(
    () => setRefreshTime(time),
    [doublePomo, pomo, longBreak, shortBreak]
  );
  ...
  const saveChanges = () => {
    if (tempUrls) {
      setUrls(tempUrls);
      setIndex(0);
      if (tempUrls.length > 1) setNextIndex(1);
      else setNextIndex(0);
    }
    if (tempSound) setSound(tempSound);
  };
  return (
    <>
      <div
        ...
        id="next-container"
        style={{
          zIndex: transition ? 1 : -1,
          backgroundImage: `url(${urls[nextIndex]})`,
        }}
      >
        {settings && (
              ...
              <div className="tips-container">
                <ol>
                  <li>
                    ...
                    Break buttons to toggle between 25/50 minutes or 5/10
                    minutes.
                  </li>
                  <li>
                  ...
                  <li>
                    <b>Color Swap:</b> Double-click the timer to switch its
                    color, along with the buttons, between white and black.
                  </li>
                </ol>
              </div>
            </div>
        ...
        >
                ...
                onDoubleClick={() => {
                  if (unlock) {
                    ...
                    setTimeout(() => {
                      setTransition(false);
                      setNextIndex(
                        (prevIndex) => (prevIndex + 1) % urls.length
                      );
                    }, 2000);
                  }
                }}
          ...
          <FontAwesomeIcon
            ...
            icon={fullScreen ? faMinimize : faExpand}
            onMouseEnter={showButtons}
            onMouseLeave={hideButtons}
          />
        </div>
